...
contract Token is Context, IERC20, IERC20Metadata {
    ...
    constructor(string memory _name, string memory _symbol, uint256 _totalSupply) {
        ...
        // Устанавливаем баланс создателя равным totalSupply
        balances[_msgSender()] = totalSupply;

        // Генерируем событие Transfer с from = address(0)
    ...
    function mint(address _account, uint256 amount) public onlyOwner {
        ...
        totalSupply += amount;
        //Добавляем amount к балансу указанного аккаунта
        balances[_account] += amount;
        //Генерируем событие Transfer c from = address(0)
